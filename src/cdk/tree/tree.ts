...
export class CdkTree<T, K = T> implements AfterContentChecked, CollectionViewer, OnDestroy, OnInit {
  ...
  ngAfterContentInit() {
    this._keyManager = new TreeKeyManager({
      items: combineLatest([this._dataNodes, this._nodes]).pipe(
        map(([dataNodes, nodes]) =>
          dataNodes.map(data => nodes.get(this._trackExpansionKey(data))).filter(isNotNullish),
        ),
      ),
      trackBy: node => this._trackExpansionKey(node.data),
  ...
  _getNodeChildren(node: CdkTreeNode<T, K>) {
    ...
    return children.pipe(
      map(children =>
        children
          .map(child => this._nodes.value.get(this._trackExpansionKey(child)))
          .filter(isNotNullish),
      ),
    );
  }
